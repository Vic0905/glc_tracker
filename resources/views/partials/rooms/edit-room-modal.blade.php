<div id="editRoomModal" class="fixed inset-0 bg-black bg-opacity-40 flex items-center justify-center z-50 hidden">
    <div class="bg-white rounded-2xl shadow-2xl p-6 sm:p-8 w-full max-w-lg mx-4 transition-all">
        ...
        <!-- Form -->
        <form id="editRoomForm" method="POST" action="" class="space-y-5">
            @csrf
            @method('PATCH')

            <!-- Room Name Input -->
            <div>
                <label for="modalRoomName" class="block text-sm font-medium text-gray-700 mb-1">Room Name</label>
                <input
                    type="text"
                    name="roomname"
                    id="modalRoomName"
