        ...
        <div class="bg-white shadow-sm sm:rounded-lg p-6">
            ...
            <div class="bg-white shadow-xl rounded-2xl overflow-hidden max-w-full max-h-[600px] overflow-y-auto text-sm font-sans">
                <table class="min-w-full border-separate border-spacing-0 text-sm">
                    <thead class="bg-slate-100 text-gray-900 sticky top-0 z-10 shadow">
                        <tr>
                            <th class="px-2 py-3 border border-gray-200 text-center text-sm">Teacher</th>
                            <th class="px-2 py-3 border border-gray-200 text-center text-sm">Room</th>
                            <th class="px-2 py-3 border border-gray-200 text-center text-sm">Date</th>
                            ...
                            @endforeach
                            @role('admin')
                                <th class="px-2 py-3 border border-gray-200 text-center text-sm">Actions</th>
                            @endrole
                        </tr>
                    </thead>
                    {{-- Table body --}}
                        ...
                        @foreach($rooms as $room)
                            ...
                            @foreach($currentRoomSchedules as $teacherDateKey => $group)
                                <tr class="hover:bg-slate-50 align-top transition text-xs">
                                    <td class="px-1 py-3 border border-gray-200 font-medium text-sm text-center">
                                        @php
                                    ...
                                    <td class="px-1 py-3 border border-gray-200 font-bold text-center">{{ $room->roomname ?? 'N/A' }}</td>
                                    <td class="px-1 py-3 border border-gray-200 font-bold text-center">
                                        ...
                                        @if($scheduledStudentsForSlot->isNotEmpty())
                                            @foreach($scheduledStudentsForSlot as $schedule)
                                                ...
                                                <div class="{{ $bgColor }} rounded-lg mb-1 p-2 shadow-sm border border-gray-200">
                                                    {{-- Name and Status (very compact, one line, potentially smaller status text) --}}
                                                    <div class="mb-0.5 text-gray-800 text-xs">
                                                        <strong class="whitespace-nowrap overflow-hidden text-ellipsis">{{ $schedule->student->name ?? 'N/A' }}</strong>
                                                        <span class="ml-0.5 px-0 py-0 inline-flex text-[0.6rem] leading-3 font-semibold rounded-full {{ $bgColor }} {{ $textColor }}">
                                                            {{ $status }}
                                                        </span>
                                                    </div>
                                                    {{-- Subject --}}
                                                    <div class="text-gray-600 text-xs flex items-center">
                                                        <svg class="w-3 h-3 mr-1 text-gray-500" fill="currentColor" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 448 512"><path d="M96 0C43 0 0 43 0 96L0 416c0 53 43 96 96 96l288 0 32 0c17.7 0 32-14.3 32-32s-14.3-32-32-32l0-64c17.7 0 32-14.3 32-32l0-320c0-17.7-14.3-32-32-32L384 0 96 0zm0 384l256 0 0 64L96 448c-17.7 0-32-14.3-32-32s14.3-32 32-32zm32-240c0-8.8 7.2-16 16-16l192 0c8.8 0 16 7.2 16 16s-7.2 16-16 16l-192 0c-8.8 0-16-7.2-16-16zm16 48l192 0c8.8 0 16 7.2 16 16s-7.2 16-16 16l-192 0c-8.8 0-16-7.2-16-16s7.2-16 16-16z" clip-rule="evenodd"></path></svg>
            ...
            <div class="flex justify-between items-center mt-4 mb-2 gap-4 flex-wrap">
                ...
                <div class="flex justify-start">
                    <a href=" {{route('schedules.available')}} " class="bg-gray-900 hover:bg-transparent px-5 py-2 text-sm shadow-sm hover:shadow-lg font-medium tracking-wider
                ...
                <div class="flex justify-between">
                    <a href=" {{route('schedules.input')}} " class="bg-gray-900 hover:bg-transparent px-5 py-2 text-sm shadow-sm hover:shadow-lg font-medium tracking-wider
                                border-2 border-gray-200 hover:border-gray-200 text-gray-100 hover:text-gray-900 rounded-lg transition ease-in duration-100">
                        Input Schedules
                    </a>
                </div>
